      ...
      res.data.forEach((el, index) => {
        ...
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-center">
          ...
          `
                        ...
                        <td class="px-6 py-4 ">
                        ` +
          (el["listening"] + el["speaking"] + el["writing"] + el["reading"]) /
            4 +
          `
                        </td>
...
function editData(id) {
  $(".submitEditBtn").click(function (e) {
    ...
    if (listening === "") {
      Toast.fire({
        icon: "error",
        title: "Hãy nhập điểm listening",
      });
    } else if (speaking === "") {
      Toast.fire({
        icon: "error",
        title: "Hãy nhập điểm speaking",
      });
    } else if (reading === "") {
      Toast.fire({
        icon: "error",
        title: "Hãy nhập điểm reading",
      });
    } else if (writing === "") {
      Toast.fire({
        icon: "error",
        title: "Hãy nhập điểm writing",
      });
    } else {
      Swal.fire({
        title: "Bạn chắc chứ?",
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "patch",
            url: "http://localhost:3000/studentjoinclasses/rating",
            headers: {
              Authorization: "Bearer " + localStorage.getItem("apitoken"),
            },
            data: {
