...
function addWarn() {
  $(".addWarn").click(function (e) {
    ...
    Swal.fire({
      title: "Bạn chắc chứ?",
      text: "Bạn đang gửi cảnh cáo cho " + temp[id]["name"],
      icon: "warning",
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          success: function (res) {
            Toast.fire({
              icon: "success",
              title: temp[id]["name"] + " đã được gửi cảnh cáo",
            });
          },
...
function addCheer() {
  $(".addCheer").click(function (e) {
    ...
    Swal.fire({
      title: "Bạn chắc chứ?",
      text: "Bạn đang gửi thông báo tuyên dương cho " + temp[id]["name"],
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "get",
          url: "http://localhost:3000/sendCheer",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("apitoken"),
          },
          data: {
          ...
          success: function (res) {
            Toast.fire({
              icon: "success",
              title: temp[id]["name"] + " đã được tuyên dương",
            });
          },
          error: function (jqXHR, textStatus, errorThrown) {
            Toast.fire({
  ...
  $(".submitEditBtn").click(function (e) {
    ...
    if (rate > 5 || rate < 0) {
      ...
      Swal.fire({
        ...
        $.ajax({
          type: "patch",
          url: "http://localhost:3000/teacherjoinclasses/rating",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("apitoken"),
          },
          data: {
