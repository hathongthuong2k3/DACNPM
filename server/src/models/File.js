...
class File {
    ...
    async addFile(teacherName, className, fileName, filetype, fileAmount, fileStatus) {
        ...
        const [result1] = await pool.query(query1, [result[0]['idTeacher'], result[0]['idClass'], fileName, filetype, fileAmount, fileStatus]);
        return result1;
    }
    async updateFile(id, fileName, filetype, fileAmount, fileStatus) {
        ...
        const [result] = await pool.query(query, [fileName, filetype, fileAmount, fileStatus, id]);
